...
이 앱은 **Week5-2 모노레포**의 일부다. 의존성과 `.env`(DATABASE_URL)는 루트에서 공유한다.

```bash
cd Week5-2          # 루트
npm install         # 최초 1회 (공용 deps: express, pg, bcryptjs, dotenv)
cp .env.example .env
#   .env 의 DATABASE_URL 을 Supabase 연결 문자열로 채운다.
#   대시보드 → Project Settings → Database → Connection string → "Connection pooling"(6543) 권장.

npm run budget      # → http://localhost:3000
```

> 로컬 PostgreSQL 로 테스트할 경우 `DATABASE_URL` 호스트가 `localhost`/`127.0.0.1` 이면 SSL 을 자동으로 끈다.
> DB 연결은 공용 `../shared/db.js` 를 사용한다.
...
```
budget-app/
├── server.js          # Express + pg(../shared/db.js), CRUD API + GROUP BY 통계
...
```
> deps·`.env`·`package.json` 은 모노레포 루트(`Week5-2/`)에서 공유한다.
