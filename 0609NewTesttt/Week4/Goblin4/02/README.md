# ✅ 할 일 관리 (React + Express + PostgreSQL)

할 일을 **추가 · 완료 토글 · 삭제** 하고 **전체 / 진행중 / 완료** 로 필터링할 수 있는 Todo 앱입니다.
모든 항목은 **Supabase PostgreSQL** 에 저장되어 앱을 껐다 켜도 그대로 유지됩니다.

![할 일 관리 스크린샷](./screenshot.png)

> 스크린샷의 목록은 데모 데이터입니다. 실제 데이터는 `.env` 의 `DATABASE_URL` 로 연결된 DB에 저장됩니다.

## 주요 기능

- ➕ 할 일 추가 (Enter 키 지원, 최대 120자)
- ✅ 완료/미완료 토글
- 🗑️ 개별 삭제 · 완료 항목 일괄 삭제
- 🔎 전체 / 진행중 / 완료 필터 탭 (각 개수 표시)
- 💬 남은 할 일 개수 배지 · 비어 있을 때 안내(Empty State)
- ⚠️ 네트워크 오류 시 5초 후 자동으로 사라지는 에러 배너
- 📱 모바일/데스크톱 반응형 + 부드러운 진입 애니메이션
...
첫 API 요청 시 `todos` 테이블이 없으면 자동 생성됩니다(`CREATE TABLE IF NOT EXISTS`).
...
| PATCH  | `/api/todos/:id`        | 완료 상태 변경 `{ done }` (없으면 토글) |
