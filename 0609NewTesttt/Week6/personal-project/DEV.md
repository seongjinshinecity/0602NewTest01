...
### Phase 2.5 — 외부 연동 검증 🟢 (완료)
- [x] Open-Meteo 날씨 API (키 불필요) 연결 + 실패 시 위젯 폴백
- [x] 매출 **수기 입력** 폼 — **7주차 파이널 v1 핵심 기능으로 완성** (입력·조회·삭제 + 대시보드 즉시 반영, `POST/GET/DELETE /api/sales`)
- ✅ 체크포인트: 날씨 API 죽어도 대시보드가 뜨는가 → 통과 / 수기 입력이 브리핑·차트에 바로 반영되는가 → 통과
