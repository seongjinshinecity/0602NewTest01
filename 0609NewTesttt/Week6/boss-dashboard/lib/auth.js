...
export function registerAuthRoutes(app) {
    ...
    try {
      ...
      try {
        const r = await pool.query(
          `INSERT INTO mall_users (email, password_hash, display_name)
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      const password = String(req.body?.password || '');
      const r = await pool.query(
        `SELECT id, email, password_hash, display_name FROM mall_users WHERE email = $1`,
        [email]
      );
      const u = r.rows[0];
